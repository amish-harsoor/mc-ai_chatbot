...
const styles = `
  ...
  .chat-window {
    ...
    right: 28px;
    width: 370px;
    /* Key: explicit height so flex children can scroll */
    height: 560px;
  ...
  .chat-header {
    ...
    /* never shrink — it's always visible */
    flex: 0 0 auto;
    position: relative;
    overflow: hidden;
  }
  ...
  /* ── Messages Area ── */
  .chat-messages {
    /* flex: 1 with min-height: 0 is the magic combo that keeps
       overflow-y: auto working inside a flex column */
    flex: 1 1 0;
    min-height: 0;
    overflow-y: auto;
    overflow-x: hidden;
  ...
  /* ─────────────────────────────────────────
     MOBILE  ≤ 600 px
     Window becomes a full-viewport sheet.
     Use inset:0 so it is never clipped or
     offset regardless of the parent's size.
  ───────────────────────────────────────── */
  @media (max-width: 600px) {
    /* FAB sits lower so thumb can reach it */
    .chat-fab {
    ...
    /* True full-screen sheet — inset:0 guarantees left/right/top/bottom */
    .chat-window {
      position: fixed;
      top: 0; right: 0; bottom: 0; left: 0;  /* inset: 0 equivalent */
      width: 100%;
      height: 100%;
      max-height: 100%;
      border-radius: 0;
    ...
    /* Push header below device status bar / notch */
    .chat-header {
      flex: 0 0 auto;
      padding: 12px 16px;
    ...
    /* Messages take all remaining vertical space */
    .chat-messages {
      flex: 1 1 0;
      min-height: 0;
    ...
    /* Push input bar above iOS home bar */
    .chat-input-area {
      flex: 0 0 auto;
      padding: 10px 14px;
...
export default function FloatingChatbot() {
    ...
    useEffect(() => {
        ...
        const initSession = async () => {
            try {
                const res = await fetch("http://10.169.20.100:8000/session/start", {
                    method: "POST",
                });
                ...
                setMessages([{ text: "Error connecting to server. Make sure backend is running.", sender: "bot", time: getTime() }]);
            }
        };
        initSession();
    }, []);
    ...
    const sendMessage = async (overrideText) => {
        ...
        try {
            const res = await fetch("http://10.169.20.100:8000/chat/stream", {
                method: "POST",
            ...
            setMessages((prev) => {
                const updated = [...prev];
                updated[updated.length - 1] = { text: "Sorry, something went wrong.", sender: "bot", time: getTime() };
                return updated;
            });
        }
        setLoading(false);
    ...
    return (
        <div className="chatbot-root">
            ...
            {/* FAB — hidden when chat is open on mobile (full screen) */}
            {(!isMobile || !open) && (
            ...
            <div className={`chat-window ${open ? "open" : "closed"}`}>
                ...
                <div className="chat-input-area">
                    ...
                    <button className="send-btn" onClick={sendMessage} disabled={!sessionId || loading} title="Send">
